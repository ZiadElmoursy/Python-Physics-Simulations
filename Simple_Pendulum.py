...
if __name__ == '__main__':
    ...
    # convert initial values from deg to rad
    theta0 = np.deg2rad(theta0)
    theta_dot0 = np.deg2rad(theta_dot0)
    
    # pendulum ODE system and numerical solution
    ...
    solution = solve_ivp(pendulum_ODE, time_span, y0,
                         t_eval=np.linspace(time_span[0],time_span[1],30*time_span[1]))
    ...
    initial conditions vector ("y0": initial angular position and velocity).
        ...
        "solve_ivp" links "t_span" to "t" and set setsthe return tuple of the
    pendulum_ODE function (y[1], -g*np.sin(y[0])/length) to be the derivative with
    respect to "t" of y = (y[0], y[1]), whose initial values are "y0".
    
        SciPy now knows that "y[1]" is the derivative of "y[0]", and that
    "-g*np.sin(y[0])/length" is the derivative of "y[1]". It logically deduces that
    "-g*np.sin(y[0])/length" is the second derivative of "y[0]", and evolves
    the system of ODEs from the initial conditions (theta0, theta_dot0).
    ...
    # convert solution from rad to degrees
    ...
    plt.xlim(time_span)
    plt.ylim(min(np.minimum(theta,theta_dot))-5, max(np.maximum(theta,theta_dot))+5)
    plt.xlabel('Time [s]')
    ...
    plt.ylim(min(theta_dot)-5, max(theta_dot)+5)
    ...
    ax.set_ylim(min(np.minimum(theta,theta_dot))-5, max(np.maximum(theta,theta_dot))+5)
    ...
    ax.set_ylim(min(theta_dot)-5, max(theta_dot)+5)
    ...
    def init_phase():
        ...
        return phase_curve, phase_point
    ...
    def animate_motion(i):
        x, y = pendulum_position(theta[i])
        line.set_data([0, x], [0, y])
        circle.set_center((x, y))
    
    animation_motion = animation.FuncAnimation(fig, animate_motion,
                                                 frames=range(len(time)))
    
    ffmpeg_writer = animation.FFMpegWriter(fps=30)
    animation_motion.save('SP_motion.mp4', writer=ffmpeg_writer)
